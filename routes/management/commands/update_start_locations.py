...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if process_all:
            routes = Route.objects.filter(start_lat__isnull=False, start_lon__isnull=False)
            ...
            self.stdout.write(f"Processing {routes.count()} routes without start_location...")
        ...
        for route in routes:
            try:
                ...
                if start_point:
                    ...
                    # No start point match, re-geocode
                    geocoded_location = get_location_name(route.start_lat, route.start_lon)
                    if geocoded_location and geocoded_location != old_location:
                        new_location = geocoded_location
                        geocoded_count += 1
                        self.stdout.write(
                            self.style.WARNING(f"  ⟳ Route #{route.id} '{route.name}': "
