...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if route_id:
            ...
            if routes.count() == 0:
                self.stdout.write(self.style.ERROR(f'Route #{route_id} not found or has no coordinates.'))
                return
            self.stdout.write(f"Processing route #{route_id}...")
        elif process_all or force:
            routes = Route.objects.exclude(route_coordinates=[])
            self.stdout.write(f"Processing all {routes.count()} routes with coordinates...")
        else:
            routes = Route.objects.exclude(route_coordinates=[]).exclude(thumbnail_image='')
            self.stdout.write(f"Processing {routes.count()} routes with existing thumbnails...")
            ...
            try:
                ...
                if not force and not route.thumbnail_image and not process_all:
                    ...
                    if options['verbosity'] >= 2:
                        self.stdout.write(f"  - Route #{route.id} '{route.name}': "
                ...
                self.stdout.write(f"  → Route #{route.id} '{route.name}': Generating thumbnail...")
                ...
                if not dry_run:
                    ...
                    thumbnail_file = generate_static_map_image(points, width=800, height=200)
                    ...
                    if thumbnail_file:
                        ...
                        success_count += 1
                        self.stdout.write(
                            self.style.SUCCESS(f"  ✓ Route #{route.id} '{route.name}': "
                                             f"Thumbnail regenerated successfully")
                        )
                    else:
                        error_count += 1
                        self.stdout.write(
                            self.style.ERROR(f"  ✗ Route #{route.id} '{route.name}': "
                ...
                error_count += 1
                self.stdout.write(
                    self.style.ERROR(f"  ✗ Route #{route.id} '{route.name}': Error - {str(e)}")
                )
